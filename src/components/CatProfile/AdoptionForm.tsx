...
import { submitAdoptionRequest, updateCatAvailability } from '../../utils/db';
...
const AdoptionForm: React.FC<AdoptionFormProps> = ({ catId, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Cambiar el estado del gato a "en_proceso"
      await updateCatAvailability(catId, 'en proceso');

      // Crear un objeto de tipo AdoptionRequest
      const adoptionRequest = {
        id: uuidv4(), // Generar un ID único para la solicitud
        catId, // Referencia al gato
        name: formData.name,
